...
export class DurationInputComponent implements OnChanges {
    ...
    durationInputForm = new FormGroup({
        days: new FormControl('', [Validators.required, Validators.min(0), Validators.max(365)]),
        hours: new FormControl('', [Validators.required, Validators.min(0), Validators.max(24)]),
